...
struct tree {
  ...
  tree() {
    cap = 0, root = new_node(), root->c[1] = new_node(),
    root->size = 2;
    root->c[1]->f = root;
  }
  ...
  //! Find the node at position pos. If sp is true, splay
  //! it.
  ...
  void erase_range(int posl, int posr) {
    ...
    n->f->c[1] = nullptr, n->f->update(), n->f->f->update(),
    n->f = nullptr;
    dealloc(n);
  }
};