...
struct kd_tree {
  ...
  struct kd_node {
    ...
    void merge(const kd_node &rhs, int k) {
      for (register int i = 0; i < k; ++i) {
        dmin.data[i] =
          std::min(dmin.data[i], rhs.dmin.data[i]);
        dmax.data[i] =
    ...
    long long min_dist(const point &rhs, int k) const {
      ...
      for (register int i = 0; i < k; ++i) {
        if (dmin.data[i] <= rhs.data[i] &&
          rhs.data[i] <= dmax.data[i])
          continue;
    ...
    long long max_dist(const point &rhs, int k) {
      ...
      for (int i = 0; i < k; ++i) {
        int tmp =
          std::max(std::abs(dmin.data[i] - rhs.data[i]),
  ...
  void _min_kth(const int &depth, const int &rt,
    ...
    if (~x && ~y &&
      sqrdist(d, tree[x].p) > sqrdist(d, tree[y].p))
      std::swap(x, y);
    if (~x &&
      ((int)heap_l.size() < m ||
        tree[x].min_dist(d, k) < heap_l.top().dist))
  ...
  void _max_kth(const int &depth, const int &rt,
    ...
    if (~x &&
      ((int)heap_r.size() < m ||
        tree[x].max_dist(d, k) >= heap_r.top().dist))
