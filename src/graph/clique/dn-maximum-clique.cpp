...
struct max_clique {
  ...
  colors QMAX, Q;
  std::vector<colors> C;
  static bool desc_degree(
    const vertex &vi, const vertex &vj) {
    return vi.d > vj.d;
  }
  ...
  void color_sort(vertices &R) {
    int j = 0, maxno = 1,
        min_k =
          std::max((int)QMAX.size() - (int)Q.size() + 1, 1);
    C[1].clear();
    C[2].clear();
  ...
  void mcqdyn(int *maxclique, int &sz) {
    set_degrees(V);
    std::sort(V.begin(), V.end(), desc_degree);
    init_colors(V);
  ...
  void degree_sort(vertices &R) {
    set_degrees(R);
    std::sort(R.begin(), R.end(), desc_degree);
  }
  max_clique(
    const BB *conn, const int sz, const float tt = .025)
