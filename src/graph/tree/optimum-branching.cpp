...
struct optimum_branching {
  ...
  void liuzhu() {
    ...
    for (int i = 2; i <= n; ++i) {
      ...
      for (int j = 1; j <= n; ++j)
        if (fa[j] != i)
          if (from[j][i] = i, edge[sel[i]][i] > edge[j][i])
            sel[i] = j;
    }
    int limit = n, prelimit = 0;
    do {
      prelimit = limit;
      ...
      for (int i = 2; i <= prelimit; ++i)
        if (fa[i] == i && !vis[i]) {
          int j = i;
          while (!vis[j]) vis[j] = i, j = getfa(sel[j]);
          if (j == 1 || vis[j] != i) continue;
          std::vector<int> C;
          int k = j;
          do C.push_back(k), k = getfa(sel[k]);
          while (k != j);
          ++limit;
          for (int i = 1; i <= n; ++i)
          ...
          for (int j = 1; j <= n; ++j)
            if (getfa(j) == limit) edge[j][limit] = INF;
          sel[limit] = 1;
          for (int j = 1; j <= n; ++j)
            if (edge[sel[limit]][limit] > edge[j][limit])
              sel[limit] = j;
        }
    } while (prelimit < limit);
