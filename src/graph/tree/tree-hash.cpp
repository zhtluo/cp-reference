...
struct tree_hash {
  ...
  tree_hash() {
    ...
    std::uniform_int_distribution<long long> uid(
      0, MOD - 1);
  ...
  struct node {
    ...
    std::pair<int, long long> del(int d, long long v) {
      if (d == d1)
        return {d2 + 1,
          mul_mod(h2, inverse(ra[d2] + v, MOD), MOD)};
      return {
        d1 + 1, mul_mod(h1, inverse(ra[d1] + v, MOD), MOD)};
    }
  };
  std::pair<int, long long> u[MAXN];
  node tree[MAXN];
  long long A[MAXN], B[MAXN];
  void dfs1(
    const edge_list<MAXN, MAXM> &e, int x, int p = -1) {
    tree[x] = node();
    for (int i = e.begin[x]; ~i; i = e.next[i]) {
      int c = e.dest[i];
      if (c != p) {
        dfs1(e, c, x);
        tree[x].add(tree[c].d1 + 1, tree[c].h1);
      }
    }
    A[x] = tree[x].hash();
  }
