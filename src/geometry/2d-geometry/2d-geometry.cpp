...
double dis2(cp a, cp b = point()) {
  return sqr(a.x - b.x) + sqr(a.y - b.y);
}

double dis(cp a, cp b = point()) {
...
bool intersect_judgment(cl a, cl b) {
  if (point_on_segment(b.s, a) || point_on_segment(b.t, a))
    return true;
...
double point_to_line(cp a, cl b) {
  return std::abs(det(b.t - b.s, a - b.s)) / dis(b.s, b.t);
}
...
std::vector<point> line_circle_intersect(cl a, cc b) {
  if (cmp(point_to_line(b.c, a), b.r) > 0)
...
std::vector<line> extangent(cc a, cc b) {
  std::vector<line> ret;
  if (cmp(dis(a.c, b.c), std::abs(a.r - b.r)) <= 0)
    return ret;
  if (sgn(a.r - b.r) == 0) {
    point dir = b.c - a.c;
    dir = (dir * a.r / dis(dir)).rot90();
    ret.push_back(line(a.c - dir, b.c - dir));
    ret.push_back(line(a.c + dir, b.c + dir));
  } else {
    point p = (b.c * a.r - a.c * b.r) / (a.r - b.r);
    std::vector<point> pp = tangent(p, a),
                       qq = tangent(p, b);
