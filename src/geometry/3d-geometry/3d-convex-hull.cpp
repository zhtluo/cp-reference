...
struct convex_hull3 {
  double mix(cp3 a, cp3 b, cp3 c) {
    return dot(det(a, b), c);
  }

  double volume(cp3 a, cp3 b, cp3 c, cp3 d) {
  ...
  struct tri {
    ...
    double area() const {
      return dis(det(p[b] - p[a], p[c] - p[a])) / 2;
    }

    point3 normal() const {
      return det(p[b] - p[a], p[c] - p[a]).unit();
    }

    double dis(cp3 p0) const {
