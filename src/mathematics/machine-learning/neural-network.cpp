template <int ft = 3, int n = 2, int m = 3,
  int MAXDATA = 100000>
struct network {
  ...
  network() {
    ...
    for (int i = 0; i < n; ++i)
      for (int j = 0; j < m; ++j)
        for (int k = 0; k < (i ? m : ft); ++k)
          wp[i][j][k] = urdn(mt);
    for (int i = 0; i < n; ++i)
      for (int j = 0; j < m; ++j) bp[i][j] = urdn(mt);
  ...
  void desc(double *x, double t, double eta) {
    ...
    for (int j = 0; j < m; ++j)
      del[n - 1][j] =
        w[j] * delo * val[n - 1][j] * (1 - val[n - 1][j]);
    for (int i = n - 2; i >= 0; --i)
      for (int j = 0; j < m; ++j) {
        del[i][j] = 0;
        for (int k = 0; k < m; ++k)
          del[i][j] += wp[i + 1][k][j] * del[i + 1][k] *
            val[i][j] * (1 - val[i][j]);
      }
    for (int j = 0; j < m; ++j) bp[0][j] -= eta * del[0][j];
    for (int j = 0; j < m; ++j)
      for (int k = 0; k < ft; ++k)
        wp[0][j][k] -= eta * del[0][j] * x[k];
    for (int i = 1; i < n; ++i)
      for (int j = 0; j < m; ++j)
        bp[i][j] -= eta * del[i][j];
    for (int i = 1; i < n; ++i)
  ...
  void train(double data[MAXDATA][ft + 1], int dn,
    ...
    for (int i = 0; i < ft + 1; ++i)
      for (int j = 0; j < dn; ++j) avg[i] += data[j][i];
    for (int i = 0; i < ft + 1; ++i) avg[i] /= dn;
    for (int i = 0; i < ft + 1; ++i)
      for (int j = 0; j < dn; ++j)
        sig[i] +=
          (data[j][i] - avg[i]) * (data[j][i] - avg[i]);
    for (int i = 0; i < ft + 1; ++i)
      sig[i] = sqrt(sig[i] / dn);
    for (int i = 0; i < ft + 1; ++i)
      for (int j = 0; j < dn; ++j)
