...
bool miller_rabin_multi(ll const &x, ...) {
  ...
  ll base;
  bool ret = true;
  while (ret && (base = va_arg(args, ll)))
    ret &= miller_rabin_single(x, base);
  va_end(args);
  return ret;
}
...
bool miller_rabin(ll const &x) {
  ...
  if (x < 4759123141ull)
    return miller_rabin_multi(x, 2, 7, 61, 0);
...
std::vector<ll> factorize(ll x) {
  ...
  trial(2);
  trial(3);
  for (l i = 5, j = 2; i < trial_limit && i * i <= x;
    i += j, j = 6 - j)
    trial(i);
  if (x > 1) {
    static std::stack<ll> s;
    s.push(x);
    while (!s.empty()) {
      x = s.top();
      s.pop();
