...
struct dft {
  ...
  void multiply(int *a, int *b, int *c, int n, int mod) {
    ...
    for (int i = 0; i < n; ++i) {
      int j = (n - i) % n;
      complex da =
                (A[i] - std::conj(A[j])) * complex(0, -0.5),
              db =
                (A[i] + std::conj(A[j])) * complex(0.5, 0),
              dc =
                (B[i] - std::conj(B[j])) * complex(0, -0.5),
              dd =
                (B[i] + std::conj(B[j])) * complex(0.5, 0);
    ...
    for (int i = 0; i < n; ++i) {
      long long da =
                  (long long)(C[i].imag() / n + 0.5) % mod,
                db =
                  (long long)(C[i].real() / n + 0.5) % mod,
                dc =
                  (long long)(D[i].imag() / n + 0.5) % mod,
                dd =
                  (long long)(D[i].real() / n + 0.5) % mod;
      c[i] =
        ((dd << (L * 2)) + ((db + dc) << L) + da) % mod;
    }
  }
};
